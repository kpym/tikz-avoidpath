...
% precision : pgfmathparse is not very precise we consides 0 <=> < \pgfeps
%  in particular two point are considered identical if the distance is < \pgfeps
% -----------------------------------------------------------
\def\pgfeps{.7}
  ...
  avoid/.style={/avoid/.cd,#1,/tikz/.cd, % set the varaibles
    to path={
      \tikzmathextra{
        ...
        if (abs(\px{projection}-\px{pole}) + abs(\py{projection}-\py{pole})) < \pgfeps then {
          ...
          if (abs(\px{first base}-\px{start}) + abs(\py{first base}-\py{start})) > \pgfeps then {
          ...
          \sameasneck = abs(veclen(\px{target}-\px{start},\py{target}-\py{start})-\avoidneck) < \pgfeps;
          if \sameasneck then {
